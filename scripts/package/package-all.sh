...
function build_zip() {
    ...
    pushd dist;
    filename="aceofwork-${version}-${platform}-${arch}.zip"
    echo "Name of package will be: $filename"

    if [[ $platform == "windows"* ]]; then
        7z a $filename aceofwork;
    else
        zip -r $filename aceofwork;
    fi
    popd;
    echo "Zip built!"
}
...
function build_setup() {
    filename="aceofwork-${version}-${platform}-${arch}-setup.exe"
    echo "Name of package will be: $filename"
    ...
    env AW_VERSION=$version_no_prefix "$innosetupdir/iscc.exe" scripts/package/aceofwork-setup.iss
    mv dist/aceofwork-setup.exe dist/$filename
    echo "Setup built!"
}
